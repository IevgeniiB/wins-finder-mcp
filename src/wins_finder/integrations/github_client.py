...
class GitHubClient:
    ...
    def get_activity(
        ...
        try:
            ...
            activity_data["summary"]["prs_merged"] = len(
                [pr for pr in prs if pr.get("merged")]
            )
            ...
            # Get issue comments
            issue_comments = self._get_issue_comments(
                github, username, start_date, end_date
            )
            ...
            activity_data["summary"]["repos_contributed"] = list(
                activity_data["summary"]["repos_contributed"]
            )

            # Cache the results
            self.db.cache_activity_data(
                "github", "activity", activity_data, start_date, end_date
            )

            logger.info(
                f"Fetched GitHub activity: {len(activity_data['activities'])} items"
            )
    ...
    def _get_pull_requests(
        self, github: Github, username: str, start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
            ...
            for pr in search_results[:DEFAULT_PR_LIMIT]:  # Limit for demo performance
                try:
                    pr_data = {
                        ...
                        "state": pr.state,
                        "merged": hasattr(pr, "pull_request")
                        and pr.pull_request
                        and hasattr(pr.pull_request, "merged_at")
    ...
    def _get_commits(
        self, github: Github, user, start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
            ...
            for repo in repos:
                try:
                    ...
                    repo_commits = repo.get_commits(
                        author=user, since=start_date, until=end_date
                    )

                    for commit in list(repo_commits)[
                        :DEFAULT_COMMIT_LIMIT
                    ]:  # Limit commits per repo
                        commits.append(
                            {
                                "type": "commit",
    ...
    def _get_issue_comments(
        ...
        try:
            ...
            for issue in list(search_results)[
                ...
                try:
                    ...
                    logger.warning(
                        f"Error processing issue comment {issue.number}: {e}"
                    )
                    continue
