...
raw_activity = Table(
    ...
    Column(
        "data_type", String(50), nullable=False
    ),  # 'prs', 'issues', 'pages', 'messages'
...
class WinsDatabase:
            ...
            if existing:
                ...
                conn.execute(
                    insert(user_preferences).values(
                        key=key, value=json.dumps(value), updated_at=func.now()
                    )
                )
            conn.commit()
    ...
    def get_preference(self, key: str, default: Any = None) -> Any:
        ...
        with self.engine.connect() as conn:
            result = conn.execute(
                select(user_preferences.c.value).where(user_preferences.c.key == key)
            )
            row = result.fetchone()
            if row:
        ...
        with self.engine.connect() as conn:
            result = conn.execute(
                insert(wins_history).values(
                    week_start=week_start, win_data_json=json.dumps(wins_data)
                )
            )
            conn.commit()
            return result.inserted_primary_key[0]
    ...
    def clear_cache(self, older_than_days: int = 7):
        ...
        with self.engine.connect() as conn:
            conn.execute(delete(raw_activity).where(raw_activity.c.timestamp < cutoff))
            conn.commit()
