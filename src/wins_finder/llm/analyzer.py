...
class WinsAnalyzer:
    ...
    def analyze_wins(
        ...
        if self._get_client():
            ...
            wins = self._heuristic_analyze_wins(activity_data, correlations, audience, focus_areas)
    ...
    def _correlate_activities(self, activity_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _group_by_time(self, activities: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def _analyze_time_group(self, activities: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def _find_keyword_correlations(self, activities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for activity in activities:
            ...
            words = title.split()
            for word in words:
                if len(word) > 4 and word not in ["pull", "request", "issue", "comment", "review"]:
    ...
    def _llm_analyze_wins(
        ...
        if not client:
            correlations = self._correlate_activities(activity_data)
            return self._heuristic_analyze_wins(activity_data, correlations, audience, focus_areas)
        ...
        prompt = self._build_correlation_and_wins_prompt(activity_summary, audience, focus_areas)
    ...
    def _prepare_raw_activity_summary(self, activity_data: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if isinstance(data, dict):
                ...
                service_summary = {
                    ...
                    "activities": [
                        {
                            "type": act.get("type"),
                            "title": act.get("title", "")[:100],  # Truncate for token efficiency
        ...
        if earliest_date and latest_date:
            summary["time_range"] = f"{earliest_date.strftime('%Y-%m-%d')} to {latest_date.strftime('%Y-%m-%d')}"
        ...
        for service, data in activity_data.items():
            if isinstance(data, dict):
                service_summary = {
                    ...
                    "key_activities": data.get("activities", [])[:5]  # First 5 activities
                }
                summary["services"][service] = service_summary
...
Focus on meaningful impact rather than just activity counts. Look for patterns that show growth, collaboration, and business value."""
    ...
    def _generate_markdown_report(self, wins_data: Dict[str, Any], audience: str) -> str:
        ...
        report_lines.append(f"- **Total Activities**: {summary.get('total_activities', 0)}")
        
        if "key_insight" in summary:
            report_lines.append(f"- **Key Insight**: {summary['key_insight']}")
        
        if "cross_service_impact" in summary:
            report_lines.append(f"- **Cross-Service Impact**: {summary['cross_service_impact']}")
        ...
        if categories:
            ...
            for category_key, category in categories.items():
                impact_emoji = "🔥" if category.get("impact") == "high" else "📈" if category.get("impact") == "medium" else "✅"
                report_lines.append(f"### {impact_emoji} {category.get('title', category_key.title())}")
                report_lines.append(f"{category.get('description', '')}")
                if category.get("evidence_count"):
                    report_lines.append(f"*Evidence: {category['evidence_count']} supporting activities*")
                report_lines.append("")
        ...
        if top_wins:
            ...
            for win in top_wins:
                impact_emoji = "🔥" if win.get("impact") == "high" else "📈" if win.get("impact") == "medium" else "✅"
                report_lines.append(f"- {impact_emoji} **{win.get('title', '')}**: {win.get('description', '')}")
        ...
        if correlations:
            ...
            for correlation in correlations:
                report_lines.append(f"- **{correlation.get('description', '')}** (confidence: {correlation.get('confidence', 0):.0%})")
    ...
    def _generate_slack_report(self, wins_data: Dict[str, Any]) -> str:
        ...
        for category_key, category in list(categories.items())[:3]:  # Top 3
            ...
            lines.append(f"{impact_emoji} {category.get('title', '')}: {category.get('description', '')}")
