...
class TestLinearClient:
    ...
    @patch.dict(os.environ, {"LINEAR_API_KEY": "test_api_key"})
    @patch("wins_finder.integrations.linear_client.requests.post")
    def test_get_created_issues(self, mock_post, temp_db):
        ...
        client = LinearClient(temp_db)
        result = client._get_created_issues("test_key", "user123", start_date, end_date)

        assert len(result) == 1
        issue = result[0]
        assert issue["type"] == "issue_created"
        assert issue["identifier"] == "TEST-1"
        assert issue["title"] == "Test issue"
        assert issue["service"] == "linear"

    @patch.dict(os.environ, {"LINEAR_API_KEY": "test_api_key"})
    @patch("wins_finder.integrations.linear_client.requests.post")
    def test_get_assigned_issues(self, mock_post, temp_db):
        ...
        client = LinearClient(temp_db)
        result = client._get_assigned_issues(
            "test_key", "user123", start_date, end_date
        )

        assert len(result) == 1
        issue = result[0]
        assert issue["type"] == "issue_updated"
        assert issue["identifier"] == "TEST-2"
        assert issue["state"] == "done"
        assert issue["service"] == "linear"

    @patch.dict(os.environ, {"LINEAR_API_KEY": "test_api_key"})
    @patch("wins_finder.integrations.linear_client.requests.post")
    def test_get_user_comments(self, mock_post, temp_db):
        ...
        client = LinearClient(temp_db)
        result = client._get_user_comments("test_key", "user123", start_date, end_date)

        assert len(result) == 1
        comment = result[0]
        assert comment["type"] == "comment"
        assert comment["body"] == "This is a test comment"
        assert comment["issue_identifier"] == "TEST-1"
        assert comment["service"] == "linear"
