...
class TestWinsAnalyzer:
    ...
    def test_analyze_wins_with_mock_llm(
    ...
    def test_generate_report_markdown(self, temp_db, sample_wins_data):
        ...
        report = analyzer.generate_report(
            sample_wins_data, format="markdown", audience="manager"
        )
    ...
    def test_generate_report_json(self, temp_db, sample_wins_data):
        ...
        report = analyzer.generate_report(
            sample_wins_data, format="json", audience="self"
        )
    ...
    def test_generate_report_slack(self, temp_db, sample_wins_data):
        ...
        report = analyzer.generate_report(
            sample_wins_data, format="slack", audience="peer"
        )
    ...
    def test_correlate_activities_multiple_services(
        self, temp_db, sample_activity_data
    ):
    ...
    def test_find_keyword_correlations_matching(self, temp_db):
        ...
        assert len(correlations) > 0
        auth_correlation = next(
            (c for c in correlations if c["keyword"] == "authentication"), None
        )
    ...
    def test_llm_analyze_wins_json_response(
        self, temp_db, sample_activity_data, mock_openai_client
    ):
        ...
        with patch.object(analyzer, "_get_client", return_value=mock_openai_client):
            result = analyzer._llm_analyze_wins(
                sample_activity_data, "manager", ["technical"]
            )

        assert result["summary"]["total_activities"] == 3
        assert "technical" in result["categories"]

    def test_llm_analyze_wins_invalid_json(
        self, temp_db, sample_activity_data, mock_openai_client
    ):
